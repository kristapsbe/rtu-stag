...
cd ../..
rm -rf process/process_func_* # get rid of all of the old files
...
for f in rtu-stag/samples/*.gz; do
    ...
    if [ ! -d "process/process_func_$sample" ]; then 
        mkdir -p "process/process_func_$sample"
        # move stag into this samples folder
        cp -r stag-mwc "process/process_func_$sample/stag-mwc"
        cp rtu-stag/configs/config.func.yaml "process/process_func_$sample/stag-mwc/config.yaml" # changing the name to the default simplifies running
        mkdir "process/process_func_$sample/stag-mwc/input"
    fi
